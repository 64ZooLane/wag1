...
module.exports = {
  	config: {
    	name: "ghost",
    	aliases: [],
		description: "ghost mentions a user in all mutual channels",
			...
			setTimeout(() => {
				...
				{ 
					...
					if (client.mode == "debug") 
						console.log(`[${i}] ghost wag1'd ${mention.username} in ${channel.guild.name}/${channel.name}`);

					if (i >= channels.length) {
						if (client.mode == "debug") console.log(`Completed ghost wag1 session for ${mention.username}`);
						client.sendContent(message.channel, message.author, `Completed ghost wag1 session for ${mention.username}`)
							.then(msg => msg.delete(5000).catch(() => {})).catch(console.error);
					}
				}).catch(console.error);
